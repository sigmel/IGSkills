...
DirectX12Shader::DirectX12Shader(IRenderDevice* device, const std::wstring& filename, const std::string& vertexFunction, const std::string& pixelFunction, const std::vector<std::string>& vertexDescription)
{
    ...
    // shaders
	D3DCompileFromFile(filename.c_str(), nullptr, nullptr, vertexFunction.c_str(), "vs_5_1", compileFlags, 0, &_vertexShaderBlob, &errorBlob);
	D3DCompileFromFile(filename.c_str(), nullptr, nullptr, pixelFunction.c_str(), "ps_5_1", compileFlags, 0, &_pixelShaderBlob, &errorBlob);
