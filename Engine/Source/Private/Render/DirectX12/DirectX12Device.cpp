...
DirectX12Device::DirectX12Device(Window* window)
{
	...
	_device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&_resourceCommandAllocator));
	_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, _resourceCommandAllocator.Get(), nullptr, IID_PPV_ARGS(&_commandList));
	_commandList->Close();
...
void DirectX12Device::SetShader(IShader* shader)
{
...
void DirectX12Device::DrawTriangles(IVertexBuffer* vertexBuffer)
{
	...
	DirectX12VertexBuffer* d3dVertexBuffer = static_cast<DirectX12VertexBuffer*>(vertexBuffer);
	_commandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	auto bufferView = d3dVertexBuffer->GetView();
	_commandList->IASetVertexBuffers(0, 1, bufferView);

	_commandList->DrawInstanced(bufferView->SizeInBytes / bufferView->StrideInBytes, 1, 0, 0);
}

void DirectX12Device::StartInitResources()
{
	assert(_threadId == this_thread::get_id());

	_resourceCommandAllocator->Reset();
	_commandList->Reset(_resourceCommandAllocator.Get(), nullptr);
}

void DirectX12Device::EndInitResources()
{
	assert(_threadId == this_thread::get_id());
...
{
	// this will reset the device for the next frame
